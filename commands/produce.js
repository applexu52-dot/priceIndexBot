...
export default async (event) => {
  try {
    ...
    // 3-1. 抓取歷史數據(3 month)
    const rssb = await yF.historical(soybeanCode, query3Month);
    console.log((rssb[rssb.length - 1].close).toFixed(4).toString());
    ...
    // 3-2. 抓取歷史數據(3year 3 month)
    const rssb2 = await yF.historical(soybeanCode, querybefore3Year);
    total = 0;
    ...
    // 3-3. 抓取歷史數據(5year 3 month)
    const rssb3 = await yF.historical(soybeanCode, querybefore5Year);
    total = 0;
...
// 3-1. 抓取歷史數據(3 month)
    ...
    soybean.body.contents[1].contents[1].text = (rssb[rssb.length - 1].close).toFixed(3).toString()
    ...
    corn.body.contents[1].contents[1].text = (rscr[rscr.length - 1].close).toFixed(3).toString()
    ...
    wheat.body.contents[1].contents[1].text = (rswht[rswht.length - 1].close).toFixed(3).toString()
